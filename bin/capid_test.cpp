...
void capid_test(Int_t run_num) {
  ...
  sprintf(root_file_name,"../dat/QIE10testing_%i_0.root",run_num);
	...
	if (lv1_mask[h][s] == 1) {
	  for (Int_t q = 0; q < QI_num; q++) {
	    if (lv2_mask[h][s][q] == 1) {
	      ...
	      if ( ( h0_temp->GetMean() != 0 ) || ( h0_temp->GetRMS() != 0 ) ) {
		lv2_err_map_rot[h][s][q] = 0;
		lv2_err_map_gen[h][s][q] = 0;
	      }	
	      if ( ( h1_temp->GetMean() != 0 ) || ( h1_temp->GetRMS() != 0 ) ) {
